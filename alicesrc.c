...
/*Determine which move does alice strategy need to use. Take argument of
*struct Player.
*/
...
void check_alice_suit(Player *p)
{
    ...
    for(index = 0; index < p->pHand; index++) {
        if(p->hand[index][0] == con.cardSuit[0]) {
            ...
            check_alice_rank(p, p->hand[index], rank, 0);
        } else if (p->hand[index][0] == con.cardSuit[1]) {
            con.numSuit[1] += 1;
            rank = p->hand[index][1] - '0';
            check_alice_rank(p, p->hand[index], rank, 1);
        } else if (p->hand[index][0] == con.cardSuit[2]) {
            con.numSuit[2] += 1;
            rank = p->hand[index][1] - '0';
            check_alice_rank(p, p->hand[index], rank, 2);
        } else if (p->hand[index][0] == con.cardSuit[3]) {
            con.numSuit[3] += 1;
            rank = p->hand[index][1] - '0';
            check_alice_rank(p, p->hand[index], rank, 3);
        }
    }
}
...
/*Check rank for alice move. Rank strategy will be different according
...
void check_alice_rank(Player *p, char *card, int rank, int pos)
{
    ...
    if(p->lead) {
        ...
        pos = lead_suit_pos(p);
        for(cardNum = 0; cardNum < p->pHand; cardNum++) {
            if(is_it_leadsuit(p, cardNum)) {
                if(con.rank[pos] == 0) {
                    con.rank[pos] = p->hand[cardNum][1];
                }
                if(p->hand[cardNum][1] - 48 < con.rank[pos]) {
                    con.rank[pos] = p->hand[cardNum][1] - 48;
                }
            }
        }
    } else if(!p->lead && !check_lead_suit(p)) {
...
void replace(Player *p)
{
    ...
    int index;
    for(index = 0; index < p->pHand; index++) {
        if((p->move[0] == p->hand[index][0]) &&
                (p->move[1] == p->hand[index][1])) {
            p->hand[index][0] = empty;
            p->hand[index][1] = empty;
            break;
        }
    }
}
